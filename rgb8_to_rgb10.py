...
# Create lookup table for maximum performance (16-bit register format)
...
def analyze_register_format(value_10bit: int) -> dict:
    """
    Analyze the 16-bit register format for a 10-bit value.

    Args:
        value_10bit: 10-bit value (0-1023)
    ...
    # Convert to 16-bit register format
    reg_16bit = value_10bit & 0x03FF

    return {
        'value_10bit': value_10bit,
        'register_16bit': reg_16bit,
        'hex_representation': f"0x{reg_16bit:04X}",
        'binary_representation': f"0b{reg_16bit:016b}",
        'used_bits': f"bits [9:0] = 0b{reg_16bit:010b}",
        'unused_bits': f"bits [15:10] = 0b{(reg_16bit >> 10):06b} (should be 000000)",
        'bit_utilization': f"{10}/16 bits used ({10/16*100:.1f}%)"
    }

def display_register_map(rgb_values_8bit: Tuple[int, int, int]) -> None:
    """
    ...
    for val8, val16, name in [(r8, r16, 'Red'), (g8, g16, 'Green'), (b8, b16, 'Blue')]:
        analysis = analyze_register_format(val16)
        print(f"\n{name} Channel:")
...
def test_conversion_16bit():
    ...
    print(f"\nMaximum 10-bit value check: 0x{1023:04X} (should be 0x03FF)")
    print(f"Register bit mask: 0x{0x03FF:04X}")

    print("\nTesting intermediate values (16-bit register format):")
    test_values = [0x00, 0x20, 0x40, 0x60, 0x80, 0xA0, 0xC0, 0xE0, 0xFF]
    for val in test_values:
        result = convert_8bit_to_10bit_16bit(val)
        print(f"0x{val:02X} -> 0x{result:04X} (decimal: {val:3d} -> {result:4d})")
...
def compare_methods_16bit():
    ...
    for val in test_values:
        ...
        print(f"0x{val:02X}  |    0x{math_result:04X}     |   0x{shift_result:04X}    | 0x{lut_result:04X}")
...
def demo_image_conversion_16bit():
    ...
    print(f"Register format: 16-bit with 10-bit data (max value: 0x{converted_image.max():04X})")
